public class Biblioteca {
    ...
    public Livro buscarLivroPorTitulo(String termoBusca) {
        termoBusca = termoBusca.toLowerCase(); // Normaliza a busca

        for (int i = 0; i < quantidadeLivros; i++) {
            String tituloLivro = livros[i].getTitulo().toLowerCase();
            if (tituloLivro.contains(termoBusca)) {
                return livros[i];
            }
        }
